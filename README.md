...
- **自定义快捷方式**：单层分组、折叠、跨组拖拽、右键管理和自定义图标；支持从网站、浏览器缓存和图标服务中在线挑选 3 个候选；删除支持撤销，所有添加入口统一校验并拦截重复网址
...
- **主题与布局**：6 套预设（云白 / 暖沙 / 石墨 / 深空蓝 / 莫兰迪绿 / 暗紫）+ 跟随系统亮暗 + 自定义主题色；页面可在 80%–125% 间整体缩放
...
├── assets/brand/           # ImageGen 品牌图标主图
└── scripts/gen-icons.mjs   # 从主图导出各尺寸 PNG（无依赖）
```
...
| 图标服务域名 | 必需 | 仅在点击「在线获取」时访问固定图标服务；不会读取目标网站页面，选中的图片保存在本机 |
